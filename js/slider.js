function slider(set) {
	...
	if(set.dots) {
		dotsCreate = function() {
			...
			arrowsDiv.appendChild(dotContainer);
	...
	if(set.numberSlid) {
		numberSlider = function(item) {
			...
			countContainer.id = ('count-slides');
			countContainer.append(sliderNumber, slash, allSliderNumber);
			sliderContainer.append(countContainer);

			var countSlidesDiv = document.getElementById('count-slides');
            var arrowsDiv = document.getElementById('arrows');

			arrowsDiv.appendChild(countSlidesDiv);
